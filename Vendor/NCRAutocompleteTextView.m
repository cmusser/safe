...
- (void)insert:(id)sender {
    if (self.autocompleteTableView.selectedRow >= 0 && self.autocompleteTableView.selectedRow < self.matches.count) {
        ...
        if ([self shouldChangeTextInRange:range replacementString:string]) {
            ...
            // Local modes for Safe. The simple setString above is done in lieu
            // of the replacement below. In this application, the entire contents
            // of the text string needs to be replaced, not just the current word.
            // Note that the range is still built and shouldChangeTextInRange is still
            // called. This was left in place to minimize changes and with the expectation
            // that shouldChangeTextInRange will continue to return YES.
            //[self replaceCharactersInRange:range withString:string];
            if ([self.delegate respondsToSelector:@selector(textInserted)]) {
                [self.delegate textInserted];
            }
